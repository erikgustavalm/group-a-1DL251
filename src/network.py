...
def run_server():
    ...
    while True:
        total_num = int( input("      - Number of Total Players(3 ~ 8): "))
        if (total_num >= 3) and (total_num <= 8):
            break
        else:
            print("      Invalid input !\n ")
    # TODO: Ask host to input number of bot players
    while True:
        real_num = int( input("      - Number of Real Players: "))
        if real_num <= total_num:
            break
        else:
            print("      Invalid input !\n ")
    
    bot_num = total_num - real_num
    print("\n      Total num: ",total_num, "(Real num: ", real_num, "/ Bot num: ", bot_num,")\n")
